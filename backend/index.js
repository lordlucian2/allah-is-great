...
// Product transformer for frontend compatibility
const transformProduct = (row) => ({
  id: String(row.id),
  name: row.name || 'Unnamed Product',
  price: parseFloat(row.price) || 0,
  originalPrice: parseFloat(row.original_price) || parseFloat(row.price) || 0,
  rating: parseFloat(row.rating) || 4.5,
  reviewsCount: parseInt(row.reviews_count) || 0,
  category: row.category || 'accessories',
  brand: row.brand || 'Others',
  image: row.image_url || 'https://placehold.co/600x400/CCCCCC/white?text=No+Image',
  images: [row.image_url || 'https://placehold.co/600x400/CCCCCC/white?text=No+Image'],
  description: row.description || '',
  specs: row.specs || {},
  isFeatured: row.is_featured || false,
  isBestSeller: row.is_best_seller || false,
  isTodayDeal: row.is_today_deal || false,
